...
# Warp images
# result12 = cv2.warpPerspective(img1, H12, (img2.shape[1], img2.shape[0]))
# result23 = cv2.warpPerspective(img3, H23, (img2.shape[1], img2.shape[0]))
# result12 = cv2.warpPerspective(img2, H12, (img2.shape[1]+img1.shape[1], img2.shape[0]+img1.shape[0]))
# result23 = cv2.warpPerspective(img3, H23, (img3.shape[1]+img2.shape[1], img3.shape[0]+img2.shape[0]))
result12 = cv2.warpPerspective(img1, H12, (img2.shape[1]+img1.shape[1], img2.shape[0]))
result23 = cv2.warpPerspective(img3, H23, (img2.shape[1]+img3.shape[1], img2.shape[0]))

# Combine images horizontally in the order 1-2-3
result = np.concatenate((result12, img2, result23), axis=1)
